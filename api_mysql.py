...
mydb = conn.connect(host="localhost", user="root", password="logan786")
...
def update():
    if request.method == 'POST':
        ...
        cursor.execute("update tasksql.mysqltable set number = number + 1 where name = %s ", (get_name,))
        mydb.commit()
...
def fetch_data():
#    get_name = request.json['get_name']
#    cursor.execute("select * from tasksql.mysqltable where name=%s", (get_name,))
